...
//         std::cout << "Css input: " << css_input[i] << std::endl;
        if(css_input[i] == '/' && CSSUtils::s_at(css_input,i+1) == '*')
        {
            astatus = PIC; ++i;
            afrom = PIS;
                             ...
                             bool& invalid_at, char& str_char, bool& pn, int str_size)
{
    ...
    std::cout << "Pin: " << pn << std::endl;
    if(is_token(css_input,i) || pn)
    {
        std::cout << "PIV " << css_input[i] << " Is Token"<< std::endl;
...
void CSSParser::parse_css(std::string css_input)
{
    ...
    css_input += "\n";
//     std::cout << "Css Input: " << css_input << std::endl;
    ...
    for(int i = 0; i < str_size; ++i)
    {
        ...
        std::cout << "Pre Status: " <<  ParseStatus[old_status] << " Current Status: " << ParseStatus[astatus]  << std::endl;
                                ...
                                std::string &css_input, int i, bool force)
{
    ...
    if (record || force) {
        ...
        sline = line;
        std::cout << "Start Posistion: " << spos << " Start Line: " << sline << std::endl;
        for(int j = i+i; j <= spos; j++)
        {
            if (css_input[j] == '\n') sline++;
        }
    }
